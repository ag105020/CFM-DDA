...
def comp(Nh):
    ...
    sup(11)
    plot(R,(Fn2fixN*QnD/Qn)/(Fn2fixN*QnC/Qn)*100,color=ColorN[Nh],label='Chain x '+str(Nh))
    if Nh == 5:
        plot(R0x,R0y,':',color='k')
    title('N transfer from the Chain',y=1.02)
    ylabel('N transfer ($\%$)')
    legend(loc=2)
    if Nh == 5:
        sf('N_transfer')
    ...
    plot(R,(FcCostChain-FphoV)/FcCostChain*100,color=ColorC[Nh],label='Chain x '+str(Nh))
    if Nh == 5:
        plot(R0x,(0,100),':',color='k')
    ...
    sup(14)
    plot(R,(Mu*QcV+Mu*QcV*E+Mu*QcH+Mu*QcH*E+Fn2fixC*QnV/Qn)/FphoV,color=ColorN[Nh],label='Chain x '+str(Nh))
    if Nh == 5:
        plot(R0x,(0,2),':',color='k')
    ...
    sup(15)
    plot(R,(Mu*QcV+Mu*QcV*E+Mu*QcH+Mu*QcH*E+Fn2fixC*QnV/Qn-FphoV)/FphoV*100,color=ColorN[Nh],label='Chain x '+str(Nh))
    if Nh == 5:
        plot(R0x,(0,60),':',color='k')
    title('$\mathit{\mu_{Chain}}$ increase in symbiosis',y=1.02)
    ylabel('$\mathit{\mu_{Chain}}$ increase ($\%$)')
#   legend(loc=4)
    if Nh == 5:
