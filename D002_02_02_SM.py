...
def comp(nNh,Nh):
    ...
    Nv = 10 #Number of the vegetative cells per heterocysts
    #Nh = 2 #Number of the heterocysts per host diatom
    
    V = arange(1000.,7000.+10.,10.)
    ...
    sup(11)
    plot(V,(Fn2fixN*QnD/Qn)/(Fn2fixN*QnC/Qn)*100,color=ColorN[nNh],label='trichome x '+str(Nh))
    if nNh == 5:
    ...
    plot(V,(FcCostChain-FphoV)/FcCostChain*100,color=ColorC[nNh],label='trichome x '+str(Nh))
    if Nv == 10:
        legend(loc=2,ncol=2,fontsize=17)
    if nNh == 5:
        #plot(V0x,(0,100),':',color='k')
        ylim(-5,105)
    title('C supply from the diatom ($\mathit{Veg}$/$\mathit{Het}$ = '+str(Nv)+')',y=1.02,fontsize=22)
    ylabel('C supply ($\%$)')
    if nNh == 5:
        sf('C_supply_from_Diatom')
    
    sup(14)
    plot(V,(Mu*QcV+Mu*QcV*E+Mu*QcH+Mu*QcH*E+Fn2fixC*QnV/Qn)/FphoV,color=ColorN[nNh],label='Chain x '+str(Nh))
    if nNh == 5:
    ...
    sup(15)
    plot(V,(Mu*(QcV+QcH)*(1+E)+Fn2fixC*(QnV+QnH)/Qn-FphoV)/FphoV*100,color=ColorN[nNh],label='Chain x '+str(Nh))
    if nNh == 5:
        plot(V0x,(0,60),':',color='k')
